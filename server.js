...
const PORT = 3000;
const JWT_SECRET = "svipe-secret-key";
...
// Заполняем username старым пользователям
const oldUsers = db
    .prepare("SELECT id, name FROM users WHERE username IS NULL OR username = ''")
    .all();

for (const user of oldUsers) {
    const username = makeUsernameFromName(user.name);
...
try {
    ...
    console.log("Username index:", e.message);
}
...
app.post("/api/register", async (req, res) => {
    try {
        ...
        await mailer.sendMail({
            ...
            text: `Ваш код подтверждения Svipe: ${code}`
        });
...
server.listen(PORT, () => {
    console.log(
        `Svipe запущен: http://localhost:${PORT}`
    );
});