...
const databasePath = path.join(__dirname, "svipe.db");
...
async function sendVerificationEmail(email, code) {
    if (
        ...
        throw new Error(
            "MAIL_USER или MAIL_PASSWORD не настроены"
        );
    }
...
app.post("/api/register", async (req, res) => {
    try {
        ...
        res.status(500).json({
            error:
                "Не удалось отправить код на почту. Проверь настройки Mail.ru."
        });
    }
});
...
app.post("/api/verify", (req, res) => {
    try {
        ...
        if (
            Date.now() >
            Number(verification.expires_at)
        ) {
            db.prepare(
                "DELETE FROM verification_codes WHERE email = ?"
            ).run(email);
    ...
    try {
        ...
        if (req.body.username !== undefined) {
            ...
            if (!isValidUsername(username)) {
                return res.status(400).json({
                    error:
                        "Username должен содержать 3-24 символа: a-z, 0-9 или _"
                });
            }
...
server.listen(
    ...
    async () => {
        ...
        try {
            ...
            console.log(
                "Server продолжает работать, но отправка кодов на email не будет работать."
            );
        }
